<? if($permissoes[23] == 'n') { erro404(); die(); }
...
if($_GET['a'] == 1) {
	if($_POST['form_processa'] == 's') {
		...
		if($prosseguir == true) {
			mysql_query("INSERT INTO menu (titulo, autor, data) VALUES ('$titulo', '$autor', '$timestamp')");
			logger("O usuário adicionou um novo menu.", "acao");
...
if($_GET['a'] == 2) {
	...
	if($_POST['form_processa'] == 's') {
		...
		if($prosseguir == true) {
			mysql_query("UPDATE menu SET titulo='$titulo' WHERE id='$id' LIMIT 1");
			logger("O usuário editou o menu #$id", "acao");
	...
	$sql3 = mysql_query("SELECT * FROM menu WHERE id='$id' LIMIT 1");
...
if($_GET['a'] == 3) {
	$id = anti_injecao($_GET['id']);
	mysql_query("DELETE FROM menu WHERE id='$id' LIMIT 1");
	logger("O usuário deletou o menu [$id]", "acao");
}
